...
export class Configuration {
    ...
    constructor() {
        this.configuration = vscode.workspace.getConfiguration('kari-move-analyzer');
    }
    ...
    get serverPath(): string {
        const defaultName = 'kari-move-analyzer';
        let serverPath = this.configuration.get<string>('server.path', defaultName);
        if (serverPath.length === 0) {
            // The default value of the `server.path` setting is 'kari-move-analyzer'.
